...
auto WebSocketHub::operator=(WebSocketHub&&) noexcept
...
auto WebSocketHub::remove_connection(ConnectionHandle conn) -> void {
  auto* crow_conn = static_cast<crow::websocket::connection*>(conn);
  std::lock_guard lock(impl_->mu);
  std::erase_if(impl_->connections, [crow_conn](const Impl::Connection& c) {
    return c.conn == crow_conn;
  });
}

auto WebSocketHub::broadcast_log(const LogMessage& msg) -> void {
  json j = {{"type", "log"},        {"timestamp", msg.timestamp},
            {"run_id", msg.run_id}, {"task_id", msg.task_id},
