...
// Declarative YAML field reader - reduces repetitive ternary operators
template <typename T>
...
// Declarative YAML field writer - only writes non-default values
template <typename T>
...
void from_yaml(const YAML::Node &node, TaskConfig &t) {
  ...
  t.cron = read_field<std::string>(node, "cron", "");
...
void to_yaml(YAML::Emitter &out, const TaskConfig &t) {
  ...
  write_field_if_not_empty(out, "cron", t.cron);
