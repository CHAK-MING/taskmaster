...
inline constexpr std::size_t MAX_OUTPUT_SIZE = 10 * 1024 * 1024; // 10 MB
...
auto read_output(int fd, std::chrono::seconds timeout,
  ...
  while (true) {
    ...
    auto result = co_await async_read(
        fd, buffer.data(), static_cast<std::uint32_t>(buffer.size()));

    if (!result) {
      auto err = result.error();
      if (err == std::errc::resource_unavailable_try_again ||
          err == std::errc::operation_would_block) {
        (void)co_await async_sleep(std::chrono::milliseconds(1));
        continue;
      }
      break;
    }
...
auto execute_command(std::string cmd, std::string working_dir,
  ...
  auto [output, timed_out] = co_await read_output(read_fd, timeout, start);
  close(read_fd);
