...
int main(int argc, char* argv[]) {
  ...
  serve->callback([&]() {
    std::exit(taskmaster::cli::cmd_serve(serve_opts));
  });
  ...
  run->callback([&]() {
    std::exit(taskmaster::cli::cmd_run(run_opts));
  });
  ...
  list->add_option("--db", list_opts.db_file, "Database file")
      ...
      ->required();
  list->callback([&]() {
    std::exit(taskmaster::cli::cmd_list(list_opts));
  });

  auto* validate = app.add_subcommand("validate", "Validate DAG files");
  validate->add_option("-c,--config", validate_opts.config_file, "System config file")
      ->check(CLI::ExistingFile)
      ->required();
  validate->callback([&]() {
